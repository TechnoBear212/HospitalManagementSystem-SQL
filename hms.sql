...
modify column patient_phone varchar(50); 

alter table patients
modify column patient_address varchar(200); 
...
FOREIGN KEY (patient_id_app) REFERENCES patients(patient_id);

ALTER TABLE appointments
ADD CONSTRAINT fk_appointments_doctors
FOREIGN KEY (doctor_id_app) REFERENCES doctors(doctor_id);
...
FOREIGN KEY (hospital_id_dep) REFERENCES hospitals(hospital_id);
...
prescription_pat_id int,
pres_doc_id int,
pres_med_id int,
prescribed_date date
); 
alter table prescriptionss rename to prescriptions; 
ALTER TABLE prescriptions
ADD CONSTRAINT fk_prescriptions_patients
FOREIGN KEY (prescription_pat_id) REFERENCES patients(patient_id);

ALTER TABLE prescriptions
ADD CONSTRAINT fk_prescriptions_doctors
FOREIGN KEY (pres_doc_id) REFERENCES doctors(doctor_id);

ALTER TABLE prescriptions
ADD CONSTRAINT fk_prescriptions_medications
FOREIGN KEY (pres_med_id) REFERENCES medications(medication_id);
...
-- inserting data 
insert into  hospitals ( hospital_id, hospital_name, address, phone) values
...
insert into doctors ( doctor_id, doctor_name, speciality, hospital_id, email) values
...
insert into patients ( patient_id, patient_name, dob, patient_address, patient_phone) values
...
insert into appointments( appointments_id, patient_id_app, doctor_id_app, appointment_date, reason) values
...
insert into departments ( department_id, hospital_id_dep, department_name) values
(1, 1, 'Cardiology'),
(2, 1, 'Radiology'),
(3, 1, 'Oncology'),
(4, 1, 'Pediatrics'),
...
insert into medications (medication_id, medication_name, description_med) values
...
insert into prescriptions (prescription_id, prescription_pat_id, pres_doc_id, pres_med_id, prescribed_date) values
...
insert into rooms ( room_id , room_name, room_dep_id, capacity) values 
(1, '101A', 3, 2),
(2, '101B', 3, 1),
(3, '102', 5, 2),
(4, '103', 5, 4),
...
(45, '704', 35, 2);
 ...
 -- patients born after the year 2000
 SELECT *
FROM patients
WHERE dob > '2000-12-31';

-- all the prescriptions for patiend id 5
select * from patients where patient_id = 5;
select * from prescriptions where prescription_pat_id =5 ;
...
-- list all medications the keywords that include pain or infection in desc
Select * from medications
WHERE description_med LIKE '%pain%'
   OR description_med LIKE '%infection%'; 
   
-- show all doctors with their hospital names
select d.doctor_id,
       d.doctor_name,
       d.speciality,
       h.hospital_name
...
join hospitals h
 ...
 -- Find the names, phones and appointment_dates of all patients with appointments in august 25
 select p.patient_name, p.patient_id from patients p 
 join appointments a
 on p.patient_id = a.patient_id_app where month(appointment_date)=8 and day(appointment_date)=25;
 
-- show all room numbers and their capaciies in the neurology department 
select * from rooms;
select r.room_id, r.room_name, r.room_dep_id, r.capacity from rooms r 
join departments d 
on r.room_dep_id = d.department_id where department_name = 'neurology' ;
...
-- FROM doctors d
-- JOIN departments de
--     ON d.hospital_id = de.hospital_id_dep
-- group by de.department_name
-- Doctors per department (given your current schema)
SELECT 
    de.department_name,
    COUNT(d.doctor_id) AS total_doctors
FROM doctors d
...
-- find pateints who have more than 3 appointments
select p.patient_name, p.patient_id from patients p
join appointments a
on p.patient_id = a.patient_id_app where patient_id_app > 3 ; 

-- list appointments with doctor and patient names
select
    a.appointments_id,
    p.patient_name,
    d.doctor_name,
...
from appointments a
join patients p
    on a.patient_id_app = p.patient_id
join doctors d
    on a.doctor_id_app = d.doctor_id
...
-- show all departments of green valley center
-- select h.hospital_id_dep from hospitals h
-- join departments d 
-- on hospital_id_dep= d.department_id
-- where h.hospital_name = "Green Valley Medical Center"
-- group by d.department_name
SELECT d.department_name
FROM hospitals h
JOIN departments d 
    ON h.hospital_id = d.hospital_id_dep
WHERE h.hospital_name = 'Green Valley Medical Center';

-- find patients who never had a prescription 
select p.patient_id, p.patient_name
from patients p
Left join prescriptions pr
    on p.patient_id = pr.prescription_pat_id
where pr.prescription_id IS NULL;

-- patients who have appointments in miore than one hospital
select p.patient_id , p.patient_name
from patients p
join appointments a
on p.patient_id = a.patient_id_app 
join doctors d
on a.doctor_id_app = d.doctor_id
join hospitals h
on d.hospital_id = h.hospital_id
group by  p.patient_id, p.patient_name
having count(distinct h.hospital_id)>1;     -- this was really hard :(
...
-- show the latest appointment for each patient
 ...
 -- 3rd most prescribed 
 SELECT m.medication_name, med_count
FROM (
    SELECT 
        pr.pres_med_id,
        COUNT(*) AS med_count,
    ...
    GROUP BY pr.pres_med_id
) t
JOIN medications m
    ON t.pres_med_id = m.medication_id
WHERE t.rnk = 3;

-- lowest hospital count 
...
HAVING doctor_count = (
    SELECT MIN(doctor_cnt)
    FROM (
        SELECT COUNT(d2.doctor_id) AS doctor_cnt
        FROM hospitals h2
        LEFT JOIN doctors d2 ON h2.hospital_id = d2.hospital_id
        GROUP BY h2.hospital_id
    ) sub
);

-- second largest capacityy
SELECT 
    h.hospital_name,
    d.department_name,
    total_capacity
FROM (
    SELECT 
        ...
        DENSE_RANK() OVER (PARTITION BY dep.hospital_id_dep ORDER BY SUM(r.capacity) DESC) AS rnk
    FROM rooms r
    JOIN departments dep
        ON r.room_dep_id = dep.department_id
    GROUP BY dep.hospital_id_dep, r.room_dep_id
) ranked
JOIN departments d
    ON ranked.room_dep_id = d.department_id
JOIN hospitals h
    ON ranked.hospital_id_dep = h.hospital_id
WHERE ranked.rnk = 2;
